...
class Human:
    def __init__(self, name, personality=None, connected_to_venomous=False, immortal=False):
        self.name = name
        self.personality = personality
        self.connected = connected_to_venomous
        self.alive = True
        self.immortal = immortal  # Infinite life flag
        self.resources = 50
        self.stability = 100
    ...
    def survive_day(self):
        ...
        if self.resources < 0:
            self.resources = 0
            self.stability -= 20
        # Immortality check: Ananthu never dies
        if self.stability <= 0 and not self.immortal:
            self.alive = False
        elif self.immortal:
            self.stability = max(self.stability, 1)  # Prevent death
...
ananthu = Human("Ananthu Sajeev", personality=personality, connected_to_venomous=True, immortal=True)
